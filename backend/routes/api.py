...
from flask import Blueprint, request, jsonify
from utils.openai_helper import obtener_respuesta_gpt, interpretar_codigos_error
from utils.obd_helper import get_obd_data, clear_obd_errors
from utils.rag_helper import process_tech_sheet, extract_specs_from_text
import logging
...
# Almacenamiento temporal de vectores (en producción usar DB)
vector_indices = {}
...
@api.route('/obd-data', methods=['GET'])
def obd_data():
    return jsonify(get_obd_data())

@api.route('/clear-errors', methods=['POST'])
def clear_errors():
    return jsonify(clear_obd_errors())

@api.route('/extract-tech-sheet', methods=['POST'])
def extract_tech_sheet():
    ...
    from utils.rag_helper import process_tech_sheet, extract_specs_from_text
    text = process_tech_sheet(path)
    if text:
        specs = extract_specs_from_text(text)
        return jsonify({"message": "Ficha procesada", "specs": specs})
    return jsonify({"error": "Error procesando"}), 500

@api.route('/analyze-part-image', methods=['POST'])
def analyze_part_image():
    if 'file' not in request.files:
        return jsonify({"error": "No file"}), 400
    # Aquí iría la integración con GPT-4 Vision
    # Por ahora simulamos la respuesta
    return jsonify({"analysis": "La pieza muestra signos de desgaste excesivo y posible corrosión en el soporte superior."})

@api.route('/diagnostico/inicio', methods=['POST'])
def iniciar_diagnostico_endpoint():
    data = request.json
    logging.debug(f"Received data: {data}")

    extracted_specs = data.get('extracted_specs', '')
    
    # Integrar datos de OBD si están presentes
    obd_info = data.get('obd_data', {})
    contexto_obd = ""
    if obd_info and (obd_info.get('status') == 'connected' or obd_info.get('rpm')):
        contexto_obd = f"Datos OBD-II en tiempo real obtenidos del vehículo: {obd_info}. " \
                       f"Usa estos valores técnicos para dar un diagnóstico preciso."

    prompt_enhancement = ""
    if extracted_specs:
        prompt_enhancement += f"\nEspecificaciones extraídas de la Ficha Técnica: {extracted_specs}"
    if contexto_obd:
        prompt_enhancement += f"\n{contexto_obd}"

    if prompt_enhancement:
        data['problema'] = f"{prompt_enhancement}\n\nProblema del usuario: {data['problema']}"

    # Validar campos requeridos básicos
    campos_requeridos = ['problema', 'marca', 'modelo', 'año']
    
    # Seleccionar campos adicionales según tipo
    tipo_vehiculo = data.get('tipo_vehiculo', 'coche')
    if tipo_vehiculo == 'coche':
        campos_requeridos.append('combustible')
    elif tipo_vehiculo == 'coche eléctrico':
...
@api.route('/diagnostico/continuar', methods=['POST'])
def continuar_diagnostico_endpoint():
    data = request.json
    logging.debug(f"Received data for continuation: {data}")

    valido, error = validar_datos(data, ['historial', 'vehicleData'])
    if not valido:
        return jsonify({"error": error}), 400
    ...
    try:
        ...
        if numero_pregunta <= MAX_PREGUNTAS:
            prompt = generar_prompt_pregunta(data['vehicleData'], historial)
            siguiente_pregunta = obtener_respuesta_gpt(prompt)
            logging.debug(f"Generated next question: {siguiente_pregunta}")
            return jsonify({
                "pregunta": siguiente_pregunta[:MAX_QUESTION_LENGTH],
            ...
            # Diagnóstico final
            tipo_vehiculo = data['vehicleData'].get('tipo_vehiculo', 'coche')
            prompt = f"Como experto en diagnóstico automotriz, realiza una evaluación final basada en la información recopilada. Proporciona un diagnóstico resumido y sugiere 2-3 posibles soluciones para el problema. Información del vehículo: {data['vehicleData']} Historial: {historial}"
            respuesta_final = obtener_respuesta_gpt(prompt)
            return jsonify({"diagnostico_y_soluciones": respuesta_final})
    except Exception as e:
        logging.error(f"Error in continuar_diagnostico: {str(e)}")
...
@api.route('/vision/analizar', methods=['POST'])
def analyze_part_image_alias():
    return analyze_part_image()
CATEGORIAS_PROBLEMAS = [
    "motor", "transmisión", "frenos", "suspensión", "eléctrico", "sobrecalentamiento", 
    "ruidos", "batería", "dirección", "climatización", "sistemas de escape", 
    "consumo de combustible", "luces", "neumáticos", "sensores", "puertas", 
